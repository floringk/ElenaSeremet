...
async function loadMembershipSignups(limit = 15): Promise<MembershipSignup[]> {
  try {
    const supabase = getSupabaseServerClient();
    ...
    if (error) {
      console.error("[admin] membership_signups load failed:", error);
      return [];
    }

    return (data || []).map((r) => ({
      id: String(r.id),
      want_goal: String(r.want_goal ?? ""),
      subscription_type: String(r.subscription_type ?? ""),
      member_kind: String(r.member_kind ?? ""),
      source_page: String(r.source_page ?? ""),
      submitted_at: String(r.submitted_at ?? "")
    }));
  } catch (error) {
    console.error("[admin] membership_signups:", error);
    return [];
  }
}
    ...
    <section className="page-section">
      <div className="container admin-dashboard">
        ...
        <div className="stats-grid">
          ...
          <article className="card">
            <h3>Tracked page views (eșantion)</h3>
            <p>{pageEvents.length}</p>
          </article>
          <article className="card">
            <h3>Înscrieri abonament (recent)</h3>
            <p>{membershipSignups.length}</p>
          </article>
        </div>
        ...
        <section className="card">
          <h2>Înscrieri abonament (studio)</h2>
          <p className="muted">Salvate în Supabase (`membership_signups`).</p>
          <div className="submission-list">
            {membershipSignups.length === 0 ? (
              <p className="muted">Nicio înscriere încă.</p>
            ) : (
              membershipSignups.map((item) => (
                <article key={item.id} className="submission-item">
                  <p>
                    <strong>{item.want_goal}</strong> · {item.subscription_type} · {item.member_kind}
                  </p>
                  <p className="muted">
                    Sursă: {item.source_page} · {new Date(item.submitted_at).toLocaleString("ro-RO")}
                  </p>
                </article>
              ))
            )}
